...
> Documento vivo de referencia. Cada componente, layout y animación documentado con fragmentos reales del código.
...
window.vfetchJSON       = function(apiUrl, path, opts) { ... }  // vfetch + .json()
...
Layout compacto centrado. El logo es la joya arriba del formulario. Sin selector de rol — el backend detecta por el formato del credential.
...
form.addEventListener('submit', async function(e) {
  ...
  // localStorage: ws_access_token, userRole, userName, userId, profile_id, userGrade
...
```js
// Lazy-load Chart.js (~215KB, solo cuando se necesita)
...
:root {
  ...
  --shadow-card: 0 4px 12px rgba(107,26,26,0.04);

  /* Dark theme (sobrescribe con .dark) */
  --bg: #0D0A08;           /* .dark */
  --text: #FFFBF0;         /* .dark */
}
```
